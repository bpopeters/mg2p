...
def read_data(path, languages=None, scripts=None, mode='wiktionary'):
    """
    ...
    assert mode in {'wiktionary', 'ipahelp'}, mode
    if mode == 'wiktionary':
...
def generate_pron_data(languages, scripts, mode):
    assert mode in {'wiktionary', 'ipahelp', 'unioned'}
    if languages == ['high']:
        languages = HIGH_RESOURCE #a little hacky
    elif languages == ['adapted']:
        languages = ADAPTED
    train_and_validate = sample(read_data(WIKTIONARY_TRAINING_PATH, languages, scripts, 'wiktionary'), 10000)
    train, validate = partition_data(train_and_validate, 0.1)
    print(train.head())
    if mode == 'ipahelp':
        train = train.reset_index().append(read_data(IPAHELP_PATH, languages, scripts, mode='ipahelp'))
        print(train.head())
